...
namespace E_commernece_app
{
    public partial class DeleteForm : Form
    {
        ...
        private void DeleteForm_Load(object sender, EventArgs e)
        {
            ...
            pDataGV.Columns.Add("Delete Product", "Delete Product");
            pDataGV.Columns["Delete Product"].Width = 100;
            ...
            foreach (Product product in AdminDL.productList)
            {
                ...
                DataGridViewButtonCell deleteButtonColumn = new DataGridViewButtonCell();
                deleteButtonColumn.Value = "Delete";
                deleteButtonColumn.UseColumnTextForButtonValue = true;
                row.Cells.Add(deleteButtonColumn);
        ...
        {
            if (e.RowIndex >= 0 && e.ColumnIndex == pDataGV.Columns["Delete Product"].Index)
            {
                int rowIndex = e.RowIndex;

                if (rowIndex < AdminDL.productList.Count)
                {
                    AdminDL.productList.RemoveAt(rowIndex);
                    pDataGV.Rows.RemoveAt(rowIndex);
                    ProductDL.storeProductDataIntoTheFile(); //update the product file as well
                    MessageBox.Show("Product deleted peramantly");
